...
OFFICE_OUTPUT_FORMAT = {"pdf"}
MEDIA_FORMATS_SET = MEDIA_FORMATS
...
class TaskManager:
    ...
    def detect_conversion_type(self, original_format: str, target_format: str) -> str:
        if original_format in OFFICE_FORMATS and target_format in OFFICE_OUTPUT_FORMAT:
    ...
    async def upload_and_start(self, db: AsyncSession, task_id: int, file_data: bytes, filename: str):
        ...
        try:
            ...
            output_path = await self._do_convert(task_id, task.conversion_type, ext, task.target_format, filename)

            output_name = Path(output_path).name
            output_minio_key = f"outputs/{task_id}/{output_name}"
            with open(output_path, "rb") as f:
                output_data = f.read()
            self.minio.upload_file(output_minio_key, output_data, self._get_content_type(task.target_format))
    ...
    async def _do_convert(self, task_id: int, conversion_type: str, original_format: str, target_format: str, filename: str) -> str:
        ...
        if conversion_type == "office_to_pdf":
            return self.converter.convert_office_to_pdf(input_path, str(output_dir))
        elif conversion_type == "media_convert":
            return self.converter.convert_media(input_path, str(output_dir), target_format)
        else:
